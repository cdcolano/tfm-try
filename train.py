...
def _generate_pseudo_labels(image1, image2, pxy_v1, encoder, projector, distributed):
    ...
    with torch.no_grad():
        ...
        if distributed:
            ...
            feats1 = encoder(image1)
            feats2 = encoder(image2)
...
def forward_vlfa(batch, models, optimizer, mode = 'wsl', distributed=True): #fsl->fully supervied, wsl->weakly supervised, sl-> supervied
    ...
    if mode in ['fsl', 'wsl']:
        ...
        if mode == 'fsl':
            ...
            pxy_c = _generate_pseudo_labels(img_v1[:B_ps], img_c[:B_ps], pxy_v1[:B_ps], encoder, projector, distributed)
